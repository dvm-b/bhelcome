...
dvmbhu = [
    "https://files.catbox.moe/xmtyq4.mp4",
    "https://files.catbox.moe/49o909.mp4",
    "https://files.catbox.moe/881cz2.mp4",
    "https://files.catbox.moe/auvjfm.mp4",
    "https://files.catbox.moe/l9n3ze.mp4",
    "https://files.catbox.moe/tx72p8.mp4",
    "https://files.catbox.moe/6smvsn.mp4"
]
...
def findtime():
    try:
        ...
        for host in ('pool.ntp.org', 'time.google.com', 'time.cloudflare.com'):
            try:
                r = c.request(host, version=3, timeout=5)
                return datetime.fromtimestamp(r.tx_time, tz=timezone.utc)
            except Exception:
                continue
        print(" ㅤㅤ\033[1;31m[ ⚚ ]    𝐔𝚗𝚎𝚡𝚙𝚎𝚌𝚝𝚎𝚍 𝐄𝚛𝚛𝚘𝚛. 𝐍𝚝𝚙 𝐅𝚎𝚝𝚌𝚑 ⚠️")
        sys.exit(1)
    except Exception:
        print(" ㅤㅤ\033[1;31m[ ⚚ ]    𝐔𝚗𝚎𝚡𝚙𝚎𝚌𝚝𝚎𝚍 𝐄𝚛𝚛𝚘𝚛. 𝐍𝚝𝚙 𝐈𝚗𝚒𝚝𝚒𝚊𝚒𝚣𝚊𝚝𝚒𝚘𝚗 ⚠️")
        sys.exit(1)
...
def wel(user, bot, status, timeleft=""):
    global file
...
def checkvip(teleid, bottoken):
    ...
    try:
        ...
        timeleft = ""
        found = False
        response = requests.get(access, timeout=15)
        ...
        while i < len(lines):
            ...
            if line == "DVMB":
                if i + 2 < len(lines):
                    ...
                    expdt = datetime.strptime(exstr, "%d/%m/%y %H:%M").replace(tzinfo=timezone.utc)
                if now <= expdt:
                    diff = expdt - now
                    timeleft = left(diff)
                    status = "𝐅𝚛𝚎𝚎 𝐕𝚎𝚛𝚜𝚒𝚘𝚗 ⭐"
        ...
        if not found:
            ...
            print(" ㅤㅤ\033[1;33m[ ⚚ ]     𝐀𝚌𝚌𝚎𝚜𝚜 𝐇𝚊𝚜 𝐄𝚡𝚙𝚒𝚛𝚎𝚍")
            print(" ㅤㅤ\033[1;33m[ ⚚ ]    𝐖𝚊𝚒𝚝 𝐅𝚘𝚛 𝐅𝚛𝚎𝚎 𝐀𝚌𝚌𝚎𝚜𝚜 𝐈𝚗 𝐂𝚑𝚊𝚗𝚗𝚎𝚕")

        msg = wel(user, bot, status, timeleft)
